...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  PRODUCT_NAME="Scribe - Insiders"
  PRODUCT_CODE="ScribeInsiders"
...
if [[ -z "${1}" ]]; then
	OUTPUT_BASE_FILENAME="Scribe-${VSCODE_ARCH}-${RELEASE_VERSION}"
else
	OUTPUT_BASE_FILENAME="Scribe-${VSCODE_ARCH}-${1}-${RELEASE_VERSION}"
fi
